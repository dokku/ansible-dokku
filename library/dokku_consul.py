...
---
module: dokku_consul
short_description: Manage the consul configuration for a given dokku application
options:
  app:
    description:
      - The name of the app
  ...
  endpoint:
    description:
      - The consul healthcheck endpoint
    required: False
    default: /
    aliases: []
  interval:
    description:
      - The consul healthcheck interval
    required: False
    default: 60s
    aliases: []
  timeout:
    description:
      - The consul healthcheck timeout
    required: True
    default: 60s
    aliases: []
  state:
...
def dokku_module_require_fields(data, required_fields):
    ...
    if len(missing_keys) > 0:
        error = 'missing required arguments: {0}'.format(
            ', '.join(missing_keys))
...
def dokku_module_absent(command_prefix, data, re_compiled, allowed_report_keys, required_present_fields, setable_fields):
    has_changed = False
    is_error = True
    meta = {'present': True, 'changed': []}

    report, error = dokku_module_report(command_prefix, data, re_compiled, allowed_report_keys)
    if error:
        meta['error'] = error
    ...
    data['enabled'] = 'false'
    has_changed, changed_keys, error = dokku_module_set_blank(command_prefix, data, setable_fields)
    if error:
        meta['error'] = error
    else:
        is_error = False
...
def dokku_module_present(command_prefix, data, re_compiled, allowed_report_keys, required_present_fields, setable_fields):
    is_error = True
    has_changed = False
    ...
    has_changed, changed_keys, error = dokku_module_set_values(command_prefix, data, report, setable_fields)
    if error:
        meta['error'] = error
    else:
        is_error = False
...
def main():
    ...
    allowed_report_keys = ['enabled', 'endpoint', 'interval', 'timeout']
    command_prefix = 'consul'
    required_present_fields = ['enabled', 'endpoint', 'interval', 'timeout']
    setable_fields = ['enabled', 'endpoint', 'interval', 'timeout']
    RE_PREFIX = re.compile('^consul-')
